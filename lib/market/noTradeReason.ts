...
function describeReason(reason: NoTradeReason): string {
  switch (reason.code) {
    ...
    case "weekend_close_blackout": {
      const directionWord = reason.impliedDirection === "long" ? "buy" : "sell";
      return `A ${directionWord} setup formed, but the weekly close is only about ${reason.hoursUntilClose} hour${reason.hoursUntilClose === 1 ? "" : "s"} away -- holding off so this doesn't sit through the weekend gap.`;
    }

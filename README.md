...
| `MAX_CORRELATED_POSITIONS` | `1` | New entries blocked outright once this many *extremely* correlated positions (a static USD-direction/commodity-complex match, or ≥0.90 real 30-day rolling correlation) are already open. Below that, correlation isn't a flat block — a new position's size is scaled down instead: 0.70–0.79 real correlation → 50% size, 0.80–0.89 → 25% size, uncapped by this setting (a correlated position is still allowed at a reduced size no matter how many others are open). See `lib/market/pairCorrelation.ts` for the static grouping and `lib/market/rollingCorrelation.ts` for the real correlation matrix/tiers. |

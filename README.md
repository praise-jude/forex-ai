...
`/dashboard` is a live SMC/ICT-style trading system for 9 pairs (see `PAIRS` in `lib/market/types.ts`): the FX majors EUR/USD, GBP/USD, USD/JPY, AUD/USD, USD/CAD, USD/CHF, and NZD/USD, plus XAU/USD and BTC/USD (crypto is exempt from the London/NY killzone gate below — see `isCrypto()` in `lib/market/symbols.ts`). The broader `Pair` type also covers XAG/USD, USOIL, UKOIL, ETH/USD, EUR/JPY, AUD/JPY, and the stock CFDs NFLX/MSFT/SPCX — untracked (no new signals), but still supported for any historical trade record or manually-opened position on one of them. It detects setups (liquidity sweep → structure break → retest of an unmitigated order block/fair value gap, during a London/NY killzone, with D1/H4/H1 trend agreement) and shows entry/SL/TP suggestions on the dashboard as Jude (long) / Omini (short) signal cards, each carrying a weighted confidence score. A score of 90%+ (95%+ for "strong buy") gets a Buy/Sell button; 80-89% is shown as "watch" — informational only, no button, since it didn't clear the bar for a real signal. By default (**Confirmation Mode**, the default manual-execution behavior — see below) **nothing is sent to the broker automatically, and clicking Buy/Sell doesn't place an order either** — it opens a Trade Proposal that must be explicitly approved (and re-clears every risk check fresh) before anything reaches MT5. Optional **DEMO**/**LIVE** engine modes make confirmed signals auto-execute with no click at all — see "Engine mode" below. See "Confirmation Mode" and "Manual execution" below before pointing this at a real account; the dashboard-only signal detection is unaffected by, and independent of, whether a signal ever gets executed.
...
**Unattended auto-execution is a separate, explicitly opt-in thing** — see "Engine mode" below (DEMO/LIVE). Confirmation Mode governs the _manual_ Buy/Sell path only.
...
Approving a Trade Proposal (or, in DEMO/LIVE mode, an auto-fired signal) runs the full risk-checked path: position size is computed to risk a fixed % of account equity per trade (never more — sizes are rounded _down_ to the broker's lot step, and a trade is skipped entirely rather than force-sized up to the broker minimum if that would exceed the configured risk). Read this whole section before setting `METAAPI_TOKEN`/`METAAPI_ACCOUNT_ID` on anything but a demo account.
...
| `UNRESTRICTED_MANUAL_TRADING`        | `false`           | When `true`, hand-built manual trades bypass account-level kill-switch, daily-loss, cooldown, max-position, max-trades, and correlation blocks. Automatic and TradingView execution remain protected; account, price, sizing, and broker validation still apply.                                                                                                                                                                                                                                                                                                                                                    |
| `MAX_CONCURRENT_POSITIONS`           | `3`               | New entries blocked once this many positions are open                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `MAX_CORRELATED_POSITIONS`           | `1`               | New entries blocked outright once this many _extremely_ correlated positions (a static USD-direction/commodity-complex match, or ≥0.90 real 30-day rolling correlation) are already open. Below that, correlation isn't a flat block — a new position's size is scaled down instead: 0.70–0.79 real correlation → 70% size, 0.80–0.89 → 60% size, uncapped by this setting (a correlated position is still allowed at a reduced size no matter how many others are open). See `lib/market/pairCorrelation.ts` for the static grouping and `lib/market/rollingCorrelation.ts` for the real correlation matrix/tiers. |
...
| `MAX_TRADES_PER_DAY`                 | `45`              | New entries blocked once this many trades have opened today (UTC day)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `M5_CONFIRMATION_ENABLED`            | `true`            | Requires the most recently closed 5-minute candle to close in a would-be signal's own direction before it fires — an on-demand REST check at decision time (see `lib/market/m5Confirmation.ts`), never a live M5 subscription (that was deliberately removed earlier as the fix for a real MetaApi rate-limit incident). On by default: this can only make execution _more_ conservative, never less.                                                                                                                                                                                                               |
| `DE_ESCALATION_ENABLED`              | `true`            | Graduated daily-loss response (see `lib/market/deEscalation.ts`): between the de-escalation threshold and the hard `MAX_DAILY_LOSS_PCT` halt, new trades open at a reduced size instead of full size. On by default — size-only, can only shrink a position, never grow one. Never applies to a manual "Edit Risk" override or an unrestricted manual trade.                                                                                                                                                                                                                                                  |
| `DE_ESCALATION_FRACTION`             | `0.5`             | Fraction of `MAX_DAILY_LOSS_PCT` at which reduced-size mode kicks in (e.g. `0.5` = half the daily-loss limit). Must be in (0, 1).                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `DE_ESCALATION_SIZE_MULTIPLIER`      | `0.5`             | Size multiplier applied while in the de-escalation band (e.g. `0.5` = half size). Clamped into (0, 1).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `ADAPTIVE_ENGINE_SIZING_ENABLED`     | `true`            | Auto-reduce position size for whichever engine (SMC / mean-reversion) shows a negative expectancy over its recent closed trades; restores full size when it recovers (see `lib/market/adaptiveEdge.ts`). Size-only, never blocks a trade, never grows past full size. Requires `EDGE_MIN_SAMPLES` closed trades in the bucket before it engages.                                                                                                                                                                                                |
| `SESSION_EDGE_SIZING_ENABLED`        | `true`            | Auto-reduce position size in a session killzone (asia/london/newyork) whose recent closed trades show a negative expectancy (see `lib/market/adaptiveEdge.ts`). Size-only, never blocks. Requires `EDGE_MIN_SAMPLES` closed trades in the bucket.                                                                                                                                                                                                                                                                                             |
| `EDGE_MIN_SAMPLES`                   | `10`              | Minimum closed trades an engine/session bucket needs before adaptive sizing engages — never act on too few trades (same posture as `CONFIDENCE_CALIBRATION_MIN_SAMPLES`). The live multiplier + reason each bucket produces is shown on `/journal` ("Adaptive sizing by engine/session").                                                                                                                                                                                                                                                    |
| `ALERT_WEBHOOK_URL`                  | unset             | Optional Telegram/Discord webhook URL for trade-event alerts (trade closed, cooldown, daily-loss halt). A second, app-independent channel alongside Expo push — a missed phone notification never means a missed trade event. Unset = disabled. See `lib/market/webhookNotifier.ts`.                                                                                                                                                                                                                                                          |
| `KILL_SWITCH_FILE`                   | `.trading-paused` | See below                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `TRADING_KILL_SWITCH`                | unset             | See below                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |

**Close All Positions**: a destructive, separately-confirmed action (next to the kill switch on `/dashboard`) that closes every currently open position on whichever account a manual click targets — pairs with the kill switch (which only stops _new_ entries) for a genuine "get me out of everything right now" escape hatch.
...
1. Set `TRADINGVIEW_WEBHOOK_SECRET` in `.env.local` to a long random value. Anyone with the URL _and_ this secret can place trades on your account — keep both private, and don't commit the secret.
